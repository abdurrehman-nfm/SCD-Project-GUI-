...
class ExpenseService {
    ...
    public void loadExpenses(DefaultListModel<String> listModel) {
        try (Scanner sc = new Scanner(new File("expenses.txt"))) {
            while (sc.hasNextLine()) {
                String[] parts = sc.nextLine().split(",");
                addExpense(parts[0], parts[1]);
                listModel.addElement(parts[0] + " - $" + parts[1]);
            }
        } catch (Exception e) {
...
public class MainFrame extends JFrame {
    ...
    public MainFrame() {
        ...
        contentPane.add(new JScrollPane(expenseList), BorderLayout.CENTER);
        ...
        addButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    listModel.addElement(categoryField.getText() + " - $" + amountField.getText());
                ...
                if (category != null && !category.trim().isEmpty()) {
                    ...
                    for (Expense exp : service.getExpenses()) {
                        if (exp.category.equalsIgnoreCase(category)) {
                            listModel.addElement(exp.toString());
                        }
                    }
                }
            }
